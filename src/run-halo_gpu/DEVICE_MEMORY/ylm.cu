...
//    float *amplitude_i_n_buff, **amplitude_i_l_buff;
    ...
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&amplitude_r_host, (l_max+1-l_init)*sizeof(float**)));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&amplitude_i_host, (l_max+1-l_init)*sizeof(float**)));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&amplitude_r_pointer, (l_max+1-l_init)*sizeof(float**)));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&amplitude_i_pointer, (l_max+1-l_init)*sizeof(float**)));

    cuda_error_flag = CUDA_CHECK_ERROR(cudaMalloc((void**)&amplitude_r, (l_max+1-l_init)*sizeof(float**)));
    if (!cuda_error_flag)
    {
...
#endif
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMalloc((void**)&amplitude_i, (l_max+1-l_init)*sizeof(float**)));
    if (!cuda_error_flag)
    {
    ...
    for(l = l_init;l <= l_max;l++)
    {
        cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&amplitude_r_pointer[l-l_init], rfunc_num[l-l_init]*sizeof(float*)));
        if (!cuda_error_flag)
        {
            printf("Memory allocation for amplitude_r_pointer[%d] failed!\n", l);
            exit(1);
        }
#ifdef DEBUG
        else
            printf("Memory allocation for amplitude_r_pointer[%d] succeed!\n", l);
#endif
        cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&amplitude_i_pointer[l-l_init], rfunc_num[l-l_init]*sizeof(float*)));
        if (!cuda_error_flag)
        {
            printf("Memory allocation for amplitude_i_pointer[%d] failed!\n", l);
            exit(1);
        }
#ifdef DEBUG
        else
            printf("Memory allocation for amplitude_i_pointer[%d] succeed!\n", l);
#endif

        cuda_error_flag = CUDA_CHECK_ERROR(cudaMalloc((void**)&amplitude_r_host[l-l_init], rfunc_num[l-l_init]*sizeof(float*)));
        if (!cuda_error_flag)
        {
            printf("Memory allocation for amplitude_r_host[%d] failed!\n", l);
            exit(1);
        }
#ifdef DEBUG
        else
            printf("Memory allocation for amplitude_r_host[%d] succeed!\n", l);
#endif
        cuda_error_flag = CUDA_CHECK_ERROR(cudaMalloc((void**)&amplitude_i_host[l-l_init], rfunc_num[l-l_init]*sizeof(float*)));
        if (!cuda_error_flag)
        {
            printf("Memory allocation for amplitude_i_host[%d] failed!\n", l);
            exit(1);
        }
#ifdef DEBUG
        else
            printf("Memory allocation for amplitude_i_host[%d] succeed!\n", l);
#endif

        for(n = 0;n < rfunc_num[l-l_init];n++)
        {
            cuda_error_flag = CUDA_CHECK_ERROR(cudaMalloc((void**)&amplitude_r_pointer[l-l_init][n], (2*l+1)*sizeof(float)));
            if (!cuda_error_flag)
            {
                printf("Memory allocation for amplitude_r_pointer[%d][%d] failed!\n", l, n);
                exit(1);
            }
#ifdef DEBUG
            else
                printf("Memory allocation for amplitude_r_pointer[%d][%d] succeed!\n", l, n);
#endif
            cuda_error_flag = CUDA_CHECK_ERROR(cudaMalloc((void**)&amplitude_i_pointer[l-l_init][n], (2*l+1)*sizeof(float)));
            if (!cuda_error_flag)
            {
                printf("Memory allocation for amplitude_i_pointer[%d][%d] failed!\n", l, n);
                exit(1);
            }
#ifdef DEBUG
            else
                printf("Memory allocation for amplitude_i_pointer[%d][%d] succeed!\n", l, n);
#endif
        }
    }
    return;
...
void ylm_fin(int l_init,int l_max)
{
	...
	int ll=l-l_init;
        ...
        {
            cuda_error_flag = CUDA_CHECK_ERROR(cudaFree(amplitude_r_pointer[ll][n]));
            cuda_error_flag = CUDA_CHECK_ERROR(cudaFree(amplitude_i_pointer[ll][n]));
        }
        free(amplitude_host[ll][0]);
        free(amplitude_host[ll]);
        cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(amplitude_r_pointer[ll]));
        cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(amplitude_i_pointer[ll]));
        cuda_error_flag = CUDA_CHECK_ERROR(cudaFree(amplitude_r_host[ll]));
        cuda_error_flag = CUDA_CHECK_ERROR(cudaFree(amplitude_i_host[ll]));
    }
    free(amplitude_host);
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(amplitude_r_pointer));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(amplitude_i_pointer));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(amplitude_r_host));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(amplitude_i_host));
...
void free_egn(int l_max,int l_init)
{
    ...
    for(int l=l_init;l<=l_max;l++)
    {
        ...
        cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(rfunc_l_pointer[l-l_init]));
        cuda_error_flag = CUDA_CHECK_ERROR(cudaFree(rfunc_pointer[l-l_init]));
    }
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(rfunc_num));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFree(rfunc_num_device));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(rv_pointer));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFree(rv));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(rfunc_l_pointer));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(rfunc_pointer));
...
void load_eigenstates(int *l_init,int *l_max, int *eigen_num, char *filename,int lnodidx)
{
    ...
    *eigen_num = 0;
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&rfunc_num, (*l_max+1-*l_init)*sizeof(int)));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&rv_pointer, (*l_max+1-*l_init)*sizeof(float*)));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&rfunc_l_pointer, (*l_max+1-*l_init)*sizeof(float**)));
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&rfunc_pointer, (*l_max+1-*l_init)*sizeof(float**)));

    cuda_error_flag = CUDA_CHECK_ERROR(cudaMalloc((void **)&rfunc_num_device, (*l_max+1-*l_init)*sizeof(int)));//rfunc_num[l] is the number of eigenvalues
    if (!cuda_error_flag)
    {
    ...
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMemcpy(rfunc_num_device, rfunc_num, (*l_max+1-*l_init)*sizeof(int), cudaMemcpyHostToDevice));
    if (!cuda_error_flag)
    {
        printf("Memory copy for eigen values numbers failed!\n");
        exit(1);
    }
#ifdef DEBUG
    else
        printf("Memory allocation for eigen values numbers succeed!\n");
#endif
    ...
    for(l = *l_init;l <= *l_max;l++)
    {
        ...
        *eigen_num += rfunc_num[ll];
        ...
        cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&rv_local_f, rfunc_num[ll]*sizeof(float)));
	...
	fread(rv_local, rfunc_num[ll], sizeof(double), pf);
        ...
        cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&rfunc_l_pointer[ll], rfunc_num[ll]*sizeof(float*)));

	for(n = 0;n < rfunc_num[ll];n++)
	{
            rv_local_f[n] = (float)(rv_local[n]);
            rfunc_local = (double*)malloc(rsiz*sizeof(double));
            cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&rfunc_local_f, rsiz*sizeof(float)));
...
#endif
            cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(rfunc_local_f));
            free(rfunc_local);
	}
        ...
        cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(rv_local_f));
        free(rv_local);
    }
...
void load_ylm(int l_init, int l_max, char *filename, int lnodidx)
{
    ...
    bool cuda_error_flag;
    FILE *pf;
    float *amplitude_r_buff, *amplitude_i_buff;
    char filen[80];
    ...
    for(l = l_init;l <= l_max;l++)
    {
        ...
        for (n=0; n<rfunc_num[l-l_init]; n++)
        {
            cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&amplitude_r_buff, (2*l+1)*sizeof(float)));
            cuda_error_flag = CUDA_CHECK_ERROR(cudaMallocHost((void**)&amplitude_i_buff, (2*l+1)*sizeof(float)));
            for (m=0; m<2*l+1; m++)
            {
//                printf("%.4e\t%.4e\n", amplitude_host[l-l_init][n][m][0], amplitude_host[l-l_init][n][m][1]);
                amplitude_r_buff[m] = (float)(amplitude_host[l-l_init][n][m][0]);
#ifdef DEBUG
                printf("Amplitude of L=%d, n=%d, m=%d have been copied to amplitude_r.\n", l, n, m);
#endif
                amplitude_i_buff[m] = (float)(amplitude_host[l-l_init][n][m][1]);
#ifdef DEBUG
                printf("Amplitude of L=%d, n=%d, m=%d have been copied to amplitude_i.\n", l, n, m);
#endif
            }
            cuda_error_flag = CUDA_CHECK_ERROR(cudaMemcpy(amplitude_r_pointer[l-l_init][n], amplitude_r_buff, (2*l+1)*sizeof(float), cudaMemcpyHostToDevice));
            cuda_error_flag = CUDA_CHECK_ERROR(cudaMemcpy(amplitude_i_pointer[l-l_init][n], amplitude_i_buff, (2*l+1)*sizeof(float), cudaMemcpyHostToDevice));
#ifdef DEBUG
            printf("Amplitude of L=%d, n= %d have been copied to amplitude_r_pointer and amplitude_i_pointer.\n", l, n);
#endif
           cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(amplitude_r_buff));
           cuda_error_flag = CUDA_CHECK_ERROR(cudaFreeHost(amplitude_i_buff));
        }
        cuda_error_flag = CUDA_CHECK_ERROR(cudaMemcpy(amplitude_r_host[l-l_init], amplitude_r_pointer[l-l_init], rfunc_num[l-l_init]*sizeof(float*), cudaMemcpyHostToDevice));
        if (!cuda_error_flag)
        {
            printf("Memory copy for amplitude_r_host[%d] failed!\n", l);
            exit(1);
        }
#ifdef DEBUG
        else
            printf("Memory copy for amplitude_r_host[%d] succeed!\n", l);
#endif
        cuda_error_flag = CUDA_CHECK_ERROR(cudaMemcpy(amplitude_i_host[l-l_init], amplitude_i_pointer[l-l_init], rfunc_num[l-l_init]*sizeof(float*), cudaMemcpyHostToDevice));
        if (!cuda_error_flag)
        {
            printf("Amplitude copy for amplitude_i_host[%d] failed!\n", l);
            exit(1);
        }
#ifdef DEBUG
        else
            printf("Amplitude copy for amplitude_i_host[%d] succeed!\n", l);
#endif
#ifdef DEBUG
        printf("Amplitude of L=%d have been copied to amplitude_r and amplitude_i.\n", l);
#endif
    }
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMemcpy(amplitude_r, amplitude_r_host, (l_max+1-l_init)*sizeof(float**), cudaMemcpyHostToDevice));
    if (!cuda_error_flag)
    {
        printf("Amplitude copy for amplitude_r[%d] failed!\n", l);
        exit(1);
    }
#ifdef DEBUG
    else
        printf("Amplitude copy for amplitude_r[%d] succeed!\n", l);
#endif
    cuda_error_flag = CUDA_CHECK_ERROR(cudaMemcpy(amplitude_i, amplitude_i_host, (l_max+1-l_init)*sizeof(float**), cudaMemcpyHostToDevice));
    if (!cuda_error_flag)
    {
        printf("Amplitude copy for amplitude_i[%d] failed!\n", l);
        exit(1);
    }
#ifdef DEBUG
    else
        printf("Amplitude copy for amplitude_i[%d] succeed!\n", l);
#endif
    fclose(pf);
//
...
//		}
//        	MPI_Barrier(MPI_COMM_WORLD);
//		MPI_Bcast(&A, 0, MPI_DOUBLE, 0, MPI_COMM_WORLD);
//	}
//        MPI_Barrier(MPI_COMM_WORLD);
//	return;
//}
