/* adding reading init_pot file name */
/* chaging the reading to take only necessary parameters (2020.12.10)*/ 
/* adding convergence check (2020.12.14)*/ 
/* adding contrast factor for determining rivial radius (2020.12.16)*/
/* calculating A (coefficient of Fermionic-King distribution function) automatically and made input parameter log (2020.12.21)*/
/* modifying lsiz calculation so sum of lsiz over all ranks equals Lsiz (2020.12.25)*/
/* adding output final potential profile as bin file (2020.12.27) */
...
//	long seed = 123456789;
	long seed = 49651525;
        ...
        printf( "Rank %5d: hostname %s\n", rank, host );

#       ifdef OCTANT_DECOMPOSE
        if ( rank == 0 )   printf( "Octant decomposition: ON\n" );
#       else
        if ( rank == 0 )   printf( "Octant decomposition: OFF\n" );
#       endif
...
//	read_para("parameter_r.txt");//read parameters
        if ( argc <= 1 )
        {
           fprintf( stderr, "ERROR : missing parameter file (mpirun -np N ./executable parameter_file) !!\n" );
           MPI_Abort( MPI_COMM_WORLD, 1 );
        }
        else if ( rank == 0 )    printf( "Parameter file = %s\n", argv[1] );
...
{
        MPI_Bcast(readname, strsiz, MPI_CHAR, 0, MPI_COMM_WORLD);
        if(strcmp(readname, paraname) == 0)
        {
            if(type == 'i')
            {
                ...
                MPI_Bcast(paravalue, 1, MPI_INT, 0, MPI_COMM_WORLD);
            }
            if(type == 'd')
            {
                if(rank == 0)
                    *((double *)paravalue) = atof(readvalue);
                MPI_Bcast(paravalue, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
            }
            if(type == 's')
            {
                if(rank == 0)
                    strcpy (paravalue, readvalue);
                MPI_Bcast(paravalue, strsiz, MPI_CHAR, 0, MPI_COMM_WORLD);
            }
        }
